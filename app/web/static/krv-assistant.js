/**
 * KRV AI Popup Assistant — vanilla embed (v=20260730q)
...
(function () {
  ...
  /**
   * Escape text, keep newlines as text (CSS white-space: pre-wrap),
   * turn http(s) URLs into safe clickable links.
   */
  function formatBotMessageHtml(text) {
    var raw = String(text == null ? "" : text);
    // Split into URL / non-URL parts without using lookbehind (older browsers)
    var re = /https?:\/\/[^\s<]+/gi;
    var html = "";
    var last = 0;
    var match;
    while ((match = re.exec(raw)) !== null) {
      ...
      var url = match[0];
      // Trim trailing punctuation often stuck to URLs in prose
      var trail = "";
      while (url.length && /[.,;:!?)]+$/.test(url)) {
        trail = url.slice(-1) + trail;
        url = url.slice(0, -1);
      }
      // Only allow http/https after trim
      if (/^https?:\/\//i.test(url)) {
        var safeHref = escapeHtml(url);
        html +=
          '<a class="krv-a-link" href="' +
          safeHref +
  ...
  function addMsg(ui, role, text) {
    var m = el("div", "krv-a-msg " + role);
    // Bot (and sys) messages: linkify URLs; user stays plain text (XSS-safe)
    if (role === "bot" || role === "sys") {
      m.innerHTML = formatBotMessageHtml(text);
    } else {
  ...
  function openPanel(ui, open) {
    if (open) ui.root.classList.add("is-open");
      ...
      ui.root.classList.remove("is-kb-open");
  ...
  function init() {
    ...
    function syncVisualViewport() {
      try {
        ...
        if (vv && vv.height) {
          ...
          if (inset > layoutH * 0.6) inset = Math.round(layoutH * 0.6);
        }
        // Prefer real VV height; never let panel think it has more room than visible
        ...
        // Keyboard UI mode: only when visualViewport actually shrinks
        // (do not key off focus alone — openChat() focuses the textarea).
        var shrunk = layoutH > 0 && h <= layoutH - 80;
        var kbOpen =
          mobile &&
          ui.root.classList.contains("is-open") &&
          (inset >= 72 || (_kbFocus && shrunk));
        if (kbOpen) ui.root.classList.add("is-kb-open");
        else ui.root.classList.remove("is-kb-open");

        // When keyboard is up, drop residual stack (CTA already hidden) so
        // panel can use almost the full visual viewport.
        if (kbOpen) {
          var cur = ui.root.style.getPropertyValue("--krv-a-stack-bottom");
          if (cur !== "8px") {
            ui.root.style.setProperty("--krv-a-stack-bottom", "8px");
          }
        }
      } catch (e) {}
    }
    ...
    function onComposerOrFormFocus(e) {
      var t = e.target;
      if (!t || !ui.root.classList.contains("is-open")) return;
      ...
      setTimeout(function () {
        try {
          syncVisualViewport();
          // Keep focused field in view (lead form scroll area or msgs)
          t.scrollIntoView({ block: "nearest", behavior: "smooth" });
          // Scroll message list to bottom so last messages stay readable
          if (ui.msgs && !ui.root.classList.contains("is-lead")) {
            ui.msgs.scrollTop = ui.msgs.scrollHeight;
          }
        } catch (err) {}
      }, 120);
    ...
    function onComposerOrFormBlur() {
      // Delay: focus may move to another field inside the widget
      setTimeout(function () {
        try {
          ...
          var still =
            ...
            !(ae.classList && ae.classList.contains("krv-a-hp"));
          if (!still) {
            _kbFocus = false;
            syncVisualViewport();
            // Restore stack for CTA offset when keyboard is gone
            updateStackOffset(ui.root);
          }
        } catch (err) {}
      }, 80);
    ...
    ui.root.addEventListener("focusin", onComposerOrFormFocus, true);
    ui.root.addEventListener("focusout", onComposerOrFormBlur, true);
